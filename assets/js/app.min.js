angular.module('app', ['ui.router', 'ngStorage', 'pascalprecht.translate', 'functions']).config(['$stateProvider', '$urlRouterProvider', '$translateProvider', '$translatePartialLoaderProvider', '$httpProvider', '$locationProvider', function ($stateProvider, $urlRouterProvider, $translateProvider, $translatePartialLoaderProvider, $httpProvider, $locationProvider) {
    $stateProvider.state('shop', { url: '/', templateUrl: './assets/partials/shop.html', controller: 'shopCtrl' }).state('product', { url: '/:title_seo/:style', templateUrl: './assets/partials/product.html', controller: 'productCtrl' }).state('cart', { url: '/cart', templateUrl: '../assets/partials/cart.html', controller: 'productCtrl' }).state('about-us', { url: '/about-us', templateUrl: '../assets/partials/about-us.html' }).state('contact-us', { url: '/contact-us', templateUrl: '../assets/partials/contact-us.html', controller: 'contactCtrl' }).state('loyalty-program', { url: '/loyalty-program', templateUrl: '../assets/partials/loyalty-program.html' }).state('terms-of-service', { url: '/terms-of-service', templateUrl: '../assets/partials/terms-of-service.html' }).state('login', { url: '/login', templateUrl: '../assets/partials/login.html', controller: 'userCtrl' }).state('signup', { url: '/signup', params: { isguest: false }, templateUrl: '../assets/partials/signup.html', controller: 'userCtrl' }).state('user', { url: '/user', templateUrl: '../assets/partials/user.html', controller: 'userCtrl' }).state('info', { url: '/info', templateUrl: '../assets/partials/info.html' }).state('printing', { url: '/printing', templateUrl: '../assets/partials/printing.html' })
    ...
    var removeHtmlFromUrl = function (page) { if (window.location.pathname === page) { window.location.href = "../"; } }
    removeHtmlFromUrl('/index.html'); $rootScope.title_seo = 'Promo Tekstil'; $scope.go = function (x) { $state.go(x); }
    $scope.goProduct = function (x) { var title_seo = functions.titleseo(functions.shortdesc(x, $rootScope.config.language.code)); $state.go('product', { title_seo: title_seo, style: x.style }); }
    if (angular.isDefined($sessionStorage.u)) { $rootScope.u = JSON.parse($sessionStorage.u); }
    ...
    $state.go('shop'); $scope.year = new Date().getFullYear(); $scope.stockNo = function (supplier) { return supplier === 'utt' ? 1 : 2; }
    ...
    $scope.isloading = false; $scope.group = ""; $scope.displayFilters = false; var type = ""; var queryString = location.search; var params = queryString.split('&'); if (params.length > 1) { var param = params[1].split('='); $scope.page = param.length === 2 ? parseInt(param[1]) : 1; } else { $scope.page = 1; }
    ...
    $scope.show = angular.isDefined($sessionStorage.config) ? $sessionStorage.config.prodctstoshow : 16; $scope.searchQuery = null; if (!functions.isNullOrEmpty(localStorage.distinct == 'null' ? null : localStorage.distinct)) { $scope.filters = true; } else { $scope.filters = false; }
    ...
    $scope.currentStep = 1; $scope.cart = !angular.isDefined(localStorage.cart) || localStorage.cart == '' ? [] : JSON.parse(localStorage.cart); $rootScope.groupingCart = !angular.isDefined(localStorage.groupingcart) || localStorage.groupingcart == '' ? [] : JSON.parse(localStorage.groupingcart); $scope.userType = 'register'; $scope.email = null; $scope.password = null; $scope.alertmsg = null; $scope.alertclass = 'info'; $scope.personType = 0; $scope.isGuest = false; if ($stateParams.isguest !== undefined) { $scope.isGuest = $stateParams.isguest; }
    $scope.goSignup = function (tpl, isGuest) { $state.go(tpl, { isguest: isGuest }); }
    var getCountries = function () { $http({ url: 'Users.asmx/GetCountries', method: 'POST', data: {} }).then(function (response) { $scope.countries = JSON.parse(response.data.d); }, function (response) { alert(JSON.parse(response.data.d)); }); }
    ...
    if (!angular.isDefined($sessionStorage.u) || $sessionStorage.u == null) { $scope.initUser($scope.isGuest); $scope.toggleTpl('registerTpl'); } else { $rootScope.u = JSON.parse($sessionStorage.u); $scope.toggleTpl('deliveryTpl'); }
    ...
    getCompanyInfo(); $scope.send = function (d) {
        ...
        $scope.issent = false; var subject = $translate.instant('inquiry') + ' - ' + $rootScope.config.appname; var body = '<p>' + $translate.instant('inquiry') + ':</p>' + '<p>' + $translate.instant('first name') + ': ' + d.firstName + '</p>' + '<p>' + $translate.instant('last name') + ': ' + d.lastName + '</p>' + '<p>' + $translate.instant('email') + ': ' + d.email + '</p>' + '<br/>' + '<p>Upit: ' + d.message + '</p>'; $http({ url: 'Mail.asmx/SendMail', method: 'POST', data: { sendTo: $rootScope.config.email, subject: subject, body: body, cc: $rootScope.config.emailcc, file: null } }).then(function (response) { $scope.issent = true; $scope.alertmsg = null; $scope.response = $translate.instant(response.data.d); }, function (response) { $scope.issent = false; $scope.alertmsg = null; alert(response.data); });
    }
